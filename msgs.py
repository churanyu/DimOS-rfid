...
class RfidTagArray:
    ...
    @classmethod
    def error(cls, message: str, *, frame_id: str = "rfid_antenna") -> RfidTagArray:
        return cls(frame_id=frame_id, status="error", message=message)
    ...
    def to_rerun(self) -> rr.TextLog:
        ...
        if self.status != "ok":
            return rr.TextLog(
                f"RFID {self.status}: {self.message}",

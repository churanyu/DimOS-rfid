...
class RfidModule(Module):
    ...
    _last_http_error_log: float = 0.0
    ...
    def _start_http_poll(self) -> None:
        ...
        if "<" in self.config.api_base or ">" in self.config.api_base:
            raise ValueError(
                "RFID_API_BASE should not include angle brackets. "
                "Use a value like http://10.42.203.26:8765/api/v1"
            )
        interval = 1.0 / self.config.poll_hz
        self._poll_http()
        self.register_disposable(
    ...
    def _poll_http(self) -> None:
        try:
            ...
            now = time.monotonic()
            if now - self._last_http_error_log > 10:
                array = RfidTagArray.error(
                    (
                        f"Cannot reach {self.config.api_base}. "
                        "Start rfid_scanner_server.py or check the robot IP/port."
                    ),
                    frame_id=self.config.antenna_frame_id,
                )
                self._latest = array
                self.rfid_tags.publish(array)
                logger.warning(
                    "RFID HTTP poll failed. Check RFID_API_BASE=%s and make sure "
                    "rfid_scanner_server.py is running: %s",
